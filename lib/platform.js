...
class SystemairPlatform {
  constructor(log, config, api) {
    ...
    this._manualFrames = Array.isArray(this.config.manualFrames) ? this.config.manualFrames : [];
    ...
    if (this._debug) {
      ...
      this.conn.on('raw', (evt) => {
        const dir = { rx: 'EMPFANGEN', tx: 'GESENDET', 'tx-manual': 'MANUELL GESENDET' }[evt.direction] || evt.direction;
    ...
    this.api.on('didFinishLaunching', () => {
      ...
      if (this._manualFrames.length > 0) this._setupManualFrameSwitches();
    });
  }
  ...
  // Legt fuer jeden in der Config unter "manualFrames" definierten Eintrag
  // einen HomeKit-Switch an. Beim Einschalten wird das dort hinterlegte
  // Hex-Frame EINMALIG gesendet (nicht der normale sendCommand-Mechanismus
  // mit Wiederholungen), danach schaltet sich der Switch selbst wieder aus
  // ("Taster"-Verhalten). Gedacht fuer gezieltes manuelles Debugging von
  // Frames, die (noch) nicht als sichere, aufgezeichnete Vorlage in
  // protocol.js hinterlegt sind.
  //
  // SICHERHEITSHINWEIS: Diese Funktion sendet exakt das eingegebene Frame,
  // ohne inhaltliche Pruefung ausser Laenge/CRC-Warnung. Ein frueherer
  // Versuch, dynamisch zusammengesetzte Frames zu senden, hat auf der
  // echten Anlage zu einem Einfrieren der Panel-Bus-Kommunikation gefuehrt
  // (siehe README). Diese Funktion ist bewusst nur ueber eine explizite,
  // vom Nutzer selbst eingetragene Config verfuegbar - niemals automatisch.
  _setupManualFrameSwitches() {
    this.log.warn(
      `[systemair] ${this._manualFrames.length} manuelle Debug-Frame(s) konfiguriert - ` +
      'diese werden UNGEPRUEFT auf den Bus gesendet, sobald der zugehoerige Schalter ' +
      'in Home betaetigt wird. Nur fuer gezieltes, bewusstes Debugging verwenden.'
    );

    this._manualFrames.forEach((entry, idx) => {
      const label = entry.name || `Debug-Frame ${idx + 1}`;
      const accessory = this._getOrCreate(
        `${this.name} ${label}`,
      ...
      const service =
        ...
        accessory.addService(this.Service.Switch, `${this.name} ${label}`);

      service.getCharacteristic(this.Characteristic.On).onSet((on) => {
        if (!on) return; // nur auf das Einschalten reagieren
        this._sendManualFrame(label, entry);
        // Taster-Verhalten: nach kurzer Zeit selbst wieder ausschalten
        setTimeout(() => {
          service.updateCharacteristic(this.Characteristic.On, false);
        }, 500);
      });
      service.getCharacteristic(this.Characteristic.On).onGet(() => false);
    });
  }

  _sendManualFrame(label, entry) {
    let frame;
    try {
      frame = proto.frameFromHex(entry.hex, { fixCrc: !!entry.fixCrc });
    } catch (err) {
      this.log.error(`[systemair] Debug-Frame "${label}" ungueltig: ${err.message}`);
      return;
    }
    if (!proto.verifyFrame(frame)) {
      this.log.warn(
        `[systemair] Debug-Frame "${label}" hat KEINE gueltige CRC (wird trotzdem gesendet, ` +
        `da manuell angefordert - setze "fixCrc": true in der Config, wenn das nicht gewollt ist)`
      );
    }
    this.log.warn(`[systemair] sende manuelles Debug-Frame "${label}" JETZT`);
    this.conn.sendRaw(frame);
  }
