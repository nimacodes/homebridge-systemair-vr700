...
// 280-Byte-Frames. Gibt {frames, rest, skipped} zurueck; `rest` ist der
// Puffer, der beim naechsten Aufruf vorangestellt werden muss. `skipped`
// zaehlt Bytes, die verworfen wurden, weil an dieser Position ein
// Header-Treffer auftrat, dessen CRC nicht passte (Rauschen/Kollision auf
// dem Bus, oder ein unbekannter/anderer Frame-Typ) - nuetzlich, um eine
// gestoerte Leitung von einer stillen zu unterscheiden.
//
...
// Baut ein Frame aus einem vom Nutzer eingegebenen Hex-String (fuer den
// manuellen Debug-/Test-Versand). Wenn fixCrc=true, wird das letzte Byte
// automatisch auf die korrekte CRC gesetzt statt den eingegebenen Wert zu
// uebernehmen - praktisch, wenn man nur an inhaltlichen Bytes herumprobiert,
// aber sich nicht selbst um die Pruefsumme kuemmern will.
function frameFromHex(hex, { fixCrc = false } = {}) {
  const clean = String(hex).replace(/[^0-9a-fA-F]/g, '');
  if (clean.length % 2 !== 0) {
    throw new Error('Hex-String hat eine ungerade Anzahl Zeichen');
  }
  const frame = Buffer.from(clean, 'hex');
  if (frame.length !== FRAME_LEN) {
    throw new Error(`Frame hat ${frame.length} Byte, erwartet werden ${FRAME_LEN}`);
  }
  if (fixCrc) {
