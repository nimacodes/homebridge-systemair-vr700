'use strict';

// Reverse-engineertes Systemair/Villavent CD-Panel-Protokoll (VR-700 DCV u.ä.)
// Laeuft ueber einen RS485<->WLAN-Adapter (z.B. Elfin EW11), der an den
// Bedienteil-Klemmen (12V/GND/Hi/Lo) angeschlossen ist. Der EW11 muss auf
// 57600 Baud, 8N1, "Protocol: None" (transparent) und "TCP Server" Port 502
// konfiguriert sein - NICHT auf "Modbus", das Geraet spricht kein Modbus.
//
// Frame-Format: 280 Byte, Header 0x55 0xAA, Byte[2] = Frame-Typ,
// letztes Byte = CRC-8 (Polynom 0x8D, init 0, keine Reflektion) ueber
// Byte[0..278].
//
// Byte 42  = Luefterstufe (0=Aus, 1=Niedrig, 2=Normal, 3=Hoch)
// Byte 95  = Temperaturstufe (0..5, sechsstufiger Handregler)
// Frame-Typ 0x0a = periodischer Zustands-Broadcast der Regelung
// Frame-Typ 0x01 = Panel-Kommando (folgt auf ein 0x02 der Regelung)

const HEADER = Buffer.from([0x55, 0xaa]);
const FRAME_LEN = 280;

// Bekannte Frame-Typen (Byte[2]).
const FRAME_TYPE = {
  COMMAND: 0x01,   // Panel-Kommando (folgt auf ein 0x02 der Regelung)
  POLL: 0x02,      // Regelung gibt dem Panel den Sende-Slot frei
  BROADCAST: 0x0a, // periodischer Zustands-Broadcast der Regelung
};

function crc8(buf, poly = 0x8d) {
  let crc = 0;
  for (let i = 0; i < buf.length; i++) {
    crc ^= buf[i];
    for (let b = 0; b < 8; b++) {
      crc = (crc & 0x80) ? ((crc << 1) ^ poly) & 0xff : (crc << 1) & 0xff;
    }
  }
  return crc;
}

function verifyFrame(frame) {
  if (frame.length !== FRAME_LEN) return false;
  if (frame[0] !== 0x55 || frame[1] !== 0xaa) return false;
  return crc8(frame.subarray(0, FRAME_LEN - 1)) === frame[FRAME_LEN - 1];
}

// Zerlegt einen eingehenden Byte-Strom in vollstaendige, CRC-gueltige
// 280-Byte-Frames. Gibt {frames, rest, skipped} zurueck; `rest` ist der
// Puffer, der beim naechsten Aufruf vorangestellt werden muss. `skipped`
// zaehlt Bytes, die verworfen wurden, weil an dieser Position ein
// Header-Treffer auftrat, dessen CRC nicht passte (Rauschen/Kollision auf
// dem Bus, oder ein unbekannter/anderer Frame-Typ) - nuetzlich, um eine
// gestoerte Leitung von einer stillen zu unterscheiden.
//
// Wichtig: der Header-Bytewert 55 AA kann durch Zufall auch INNERHALB der
// Nutzdaten eines Frames auftreten. Ein naiver Split "zwischen zwei
// Vorkommen von 55 AA" erzeugt dadurch falsche Kurz-Frames. Deshalb wird
// hier an jeder Header-Position die CRC ueber die vollen 280 Byte geprueft;
// nur bei gueltiger CRC wird das Frame akzeptiert und um FRAME_LEN
// vorgerueckt. Bei ungueltiger CRC (falscher Positivtreffer) wird nur 1
// Byte vorgerueckt und die Suche nach dem naechsten Header fortgesetzt.
function splitFrames(buffer) {
  const frames = [];
  let offset = 0;
  let skipped = 0;

  while (true) {
    const idx = buffer.indexOf(HEADER, offset);
    if (idx === -1) {
      offset = buffer.length;
      break;
    }
    if (idx + FRAME_LEN > buffer.length) {
      offset = idx;
      break;
    }
    const candidate = buffer.subarray(idx, idx + FRAME_LEN);
    if (verifyFrame(candidate)) {
      frames.push(Buffer.from(candidate));
      offset = idx + FRAME_LEN;
    } else {
      skipped += 1;
      offset = idx + 1;
    }
  }

  return { frames, rest: Buffer.from(buffer.subarray(offset)), skipped };
}

// Baut ein Frame aus einem vom Nutzer eingegebenen Hex-String (fuer den
// manuellen Debug-/Test-Versand). Wenn fixCrc=true, wird das letzte Byte
// automatisch auf die korrekte CRC gesetzt statt den eingegebenen Wert zu
// uebernehmen - praktisch, wenn man nur an inhaltlichen Bytes herumprobiert,
// aber sich nicht selbst um die Pruefsumme kuemmern will.
function frameFromHex(hex, { fixCrc = false } = {}) {
  const clean = String(hex).replace(/[^0-9a-fA-F]/g, '');
  if (clean.length % 2 !== 0) {
    throw new Error('Hex-String hat eine ungerade Anzahl Zeichen');
  }
  const frame = Buffer.from(clean, 'hex');
  if (frame.length !== FRAME_LEN) {
    throw new Error(`Frame hat ${frame.length} Byte, erwartet werden ${FRAME_LEN}`);
  }
  if (fixCrc) {
    frame[FRAME_LEN - 1] = crc8(frame.subarray(0, FRAME_LEN - 1));
  }
  return frame;
}

function frameType(frame) {
  if (!frame || frame.length < 3) return null;
  return frame[2];
}

function isCommandFrame(frame) {
  return verifyFrame(frame) && frame[2] === FRAME_TYPE.COMMAND;
}

function isBroadcastFrame(frame) {
  return verifyFrame(frame) && frame[2] === FRAME_TYPE.BROADCAST;
}

function parseState(frame) {
  if (!verifyFrame(frame) || frame[2] !== FRAME_TYPE.BROADCAST) return null;
  return {
    fanSpeed: frame[42],       // 0..3
    tempLevel: frame[95],      // 0..5
  };
}

// Bekannte, real aufgezeichnete Typ-0x01-Kommandoframes (gueltige CRC).
// Jedes Frame setzt EINEN der beiden Regler; der jeweils andere Wert im
// Frame spiegelt den Zustand zum Aufnahmezeitpunkt, wird aber von der
// Regelung offenbar nicht als Widerspruch gewertet - nur der eigentliche
// Zielwert (Byte 42 bzw. 95) wird uebernommen.
const FAN_SPEED_CMD = {
  0: '',
  1: '',
  2: '',
  3: '',
};

const TEMP_LEVEL_CMD = {
  0: '',
  1: '',
  2: '',
  3: '',
  4: '',
  5: '',
};

function commandForFanSpeed(level) {
  const hex = FAN_SPEED_CMD[level];
  if (!hex) throw new Error(`Unbekannte Luefterstufe: ${level}`);
  return Buffer.from(hex, 'hex');
}

function commandForTempLevel(level) {
  const hex = TEMP_LEVEL_CMD[level];
  if (!hex) throw new Error(`Unbekannte Temperaturstufe: ${level}`);
  return Buffer.from(hex, 'hex');
}

// HINWEIS: Frueher gab es hier eine commandForState()-Funktion, die ein Frame
// dynamisch aus einem TEMP-Template mit ueberschriebenen Fan-Bytes
// zusammensetzte ("Frankenstein"-Frame). Ihr Einsatz hat auf der echten Anlage
// (VR-700) zu einem Absturz/Reset der Regelungsplatine gefuehrt. Sie wurde
// bewusst vollstaendig entfernt: Es werden ausschliesslich exakte,
// unveraenderte Frames gesendet - entweder die hier hinterlegten Vorlagen oder
// die per Lernmodus (lib/learning.js) von der eigenen Anlage aufgezeichneten.
// Wer das Protokoll erweitert, sollte NIE einzelne Bytes eines Frames aendern,
// solange nicht alle 280 Byte verstanden sind.

module.exports = {
  HEADER,
  FRAME_LEN,
  FRAME_TYPE,
  crc8,
  verifyFrame,
  splitFrames,
  parseState,
  frameType,
  isCommandFrame,
  isBroadcastFrame,
  commandForFanSpeed,
  commandForTempLevel,
  frameFromHex,
};
